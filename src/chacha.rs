...
impl StreamCipher {
    ...
    fn store_rate(mut self, out: &mut [u8], block_offset: u64) {
        self.st[14] = block_offset as _;
        self.st[15] = (block_offset >> 32) as _;
        self.permute();
        out[0 * 4..][0..4].copy_from_slice(&(self.st[0]).to_le_bytes());
        out[1 * 4..][0..4].copy_from_slice(&(self.st[1]).to_le_bytes());
        out[2 * 4..][0..4].copy_from_slice(&(self.st[2]).to_le_bytes());
        out[3 * 4..][0..4].copy_from_slice(&(self.st[3]).to_le_bytes());
        out[4 * 4..][0..4].copy_from_slice(&(self.st[12]).to_le_bytes());
        out[5 * 4..][0..4].copy_from_slice(&(self.st[13]).to_le_bytes());
        out[6 * 4..][0..4].copy_from_slice(&(self.st[14]).to_le_bytes());
        out[7 * 4..][0..4].copy_from_slice(&(self.st[15]).to_le_bytes());
    }
    ...
    fn apply_rate(mut self, out: &mut [u8], block_offset: u64) {
        ...
        out[4 * 4..][0..4].copy_from_slice(&(out4 ^ self.st[12]).to_le_bytes());
        out[5 * 4..][0..4].copy_from_slice(&(out5 ^ self.st[13]).to_le_bytes());
        out[6 * 4..][0..4].copy_from_slice(&(out6 ^ self.st[14]).to_le_bytes());
        out[7 * 4..][0..4].copy_from_slice(&(out7 ^ self.st[15]).to_le_bytes());
    }

    /// Squeeze and return a 32-byte block.
    #[inline(always)]
    fn squeeze_rate(self, block_offset: u64) -> [u8; 32] {
        let mut out = [0u8; 32];
    ...
    pub fn fill(&self, mut out: &mut [u8], start_offset: u64) -> Result<(), &'static str> {
        ...
        let mut block_offset = start_offset / 32;
        let offset_in_first_block = (start_offset % 32) as usize;
        let bytes_to_copy = cmp::min(32 - offset_in_first_block, out.len());
        if bytes_to_copy > 0 {
        ...
        while out.len() >= 32 {
            block_offset += 1;
            self.store_rate(&mut out[..32], block_offset);
            out = &mut out[32..];
        }
        if !out.is_empty() {
            block_offset += 1;
    ...
    pub fn apply_keystream(
        ...
        while out.len() >= 32 {
            block_offset += 1;
            self.apply_rate(&mut out[..32], block_offset);
            out = &mut out[32..];
        }
        if !out.is_empty() {
            block_offset += 1;
